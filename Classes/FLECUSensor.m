...
+ (NSArray*) troubleCodesForResponse:(FLScanToolResponse*)response {
	...
	@try {
		...
		if(NULL == data || dataLength < 2) {
			...
			FLERROR(@"data (0x%08X) is NULL or dataLength is not a multiple of 2 (%d)", (unsigned int)data, dataLength)
...
- (NSString*) minValueStringForMeasurement1:(BOOL)metric {
	...
	if(minValue != INT_MAX) {
		return [NSString stringWithFormat:@"%0.1ld", (long)minValue];
	}
	else {
		return nil;
	}
...
- (NSString*) maxValueStringForMeasurement1:(BOOL)metric {
	...
	if(maxValue != INT_MAX) {
		if(maxValue > 1000) {
			...
			return [NSString stringWithFormat:@"%0.1ld", (long)maxValue];
		}		
	}
	else {
		return nil;
